...
public class CaptureService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (Actions.ACTION_START_PROJECTION.equals(action)) {
            ...
            ensureForegroundIfNeeded("陪看模式运行中");
            startPeriodic();
            ...
            ensureForegroundIfNeeded("悬浮头像已开启");
            showFloatingButton();
        } else if (Actions.ACTION_HIDE_FLOATING.equals(action)) {
            hideFloatingButton();
        } else {
            ensureForegroundIfNeeded("给你看看运行中");
        }
        return START_STICKY;
    }
    ...
    private void captureNow(int attempt) {
        ...
        try {
            ...
            if (image == null) {
                if (attempt < 3) {
                    captureHandler.postDelayed(() -> captureNow(attempt + 1), 180);
                    return;
                }
                busy.set(false);
    ...
    private Notification buildNotification(String title, String text) {
        ...
        PendingIntent capturePi = PendingIntent.getService(this, 2, captureIntent,
                PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE);
    ...
    private void startForegroundForProjection(String title) {
    ...
    private void ensureForegroundIfNeeded(String title) {
        Notification n = buildNotification(title, lastReply == null || lastReply.isEmpty() ? "等待操作" : lastReply);
        try {
